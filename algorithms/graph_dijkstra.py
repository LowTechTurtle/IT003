class Edge(object):
    ...
    def set_vertices(self, tail, head, weight):
        self.tail = tail
        self.head = head
        self.weight = weight
...
class ALGraph(object):
    ...
    def add_vertex(self, from_vertex, new_vertex, edge_weight, inv=False):
        self.vertices.append(new_vertex)
        if inv:
            edge = Edge(new_vertex, from_vertex, edge_weight)
        else:
            edge = Edge(from_vertex, new_vertex, edge_weight)

        self.edge.append(edge)
        new_vertex.add_edge(edge)
        from_vertex.add_edge(edge)

    def new_edge(self, from_vertex, to_vertex, edge_weight, inv=False):
        if inv:
            edge = Edge(to_vertex, from_vertex, edge_weight)
        else:
            edge = Edge(from_vertex, to_vertex, edge_weight)
